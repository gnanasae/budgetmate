...
const App = () => {
  ...
  const { incomes, expenses, debts, savingsGoals, investments } = useMemo(() => {
    ...
    return {
        incomes: filter(allData[selectedMonth]?.incomes, ['source', 'category']),
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      case 'dashboard': return <DashboardTab allData={allData} totalIncome={totalIncome} totalExpenses={totalExpenses} {...props} />;
